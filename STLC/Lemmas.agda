...
rr-eq eqv (` x) = eqv x
rr-eq eqv true = refl
rr-eq eqv false = refl
rr-eq eqv (t · s) = app-term-≡ (rr-eq eqv t) (rr-eq eqv s)
rr-eq eqv (if t then t₁ else t₂) = if-term-≡ (rr-eq eqv t) (rr-eq eqv t₁) (rr-eq eqv t₂)
...
rename-lift-weaken≡weaken-rename' ρ (su x) = (` lookupRen (lookupRen x (mapRen su (mapRen su idRen))) (lift ρ))
                                           ...
                                           ≡⟨ ≡-sym (weaken`≡`su (lookupRen (su x) ρ)) ⟩ weaken (` lookupRen (su x) ρ) ∎
...
sr-rs-eq eqv true = refl
sr-rs-eq eqv false = refl
sr-rs-eq eqv (t · s) = app-term-≡ (sr-rs-eq eqv t) (sr-rs-eq eqv s)
sr-rs-eq eqv (if t then t₁ else t₂) = if-term-≡ (sr-rs-eq eqv t) (sr-rs-eq eqv t₁) (sr-rs-eq eqv t₂)
...
subst-rename≡rename-subst' ρ {s} (su x) = lookup (lookupRen x (mapRen su ρ)) (rename ρ s ∷ idSub)
                                        ...
                                        ≡⟨ lookup-idSub (lookupRen x ρ) ⟩ 
                                         (` lookupRen x ρ)
                                        ≡⟨ refl ⟩
                                          rename ρ (` x)
                                        ≡⟨ cong (rename ρ) (≡-sym (lookup-idSub x)) ⟩ 
...
subsub (if t then t₁ else t₂) = if-term-≡ (subsub t) (subsub t₁) (subsub t₂)

sr-s-eq : {ρ : Ren Γ Δ}{ts : Sub Δ Θ}{ss : Sub Γ Θ}
          → (∀{σ} → (x : Γ ∋ σ) → subst (rename ρ (` x)) ts ≡ subst (` x) ss)
          → {σ : Type}(t : Γ ⊢ σ) → subst (rename ρ t) ts ≡ subst t ss
sr-s-eq eqv (` x) = eqv x
sr-s-eq eqv true = refl
sr-s-eq eqv false = refl
sr-s-eq eqv (t · s) = app-term-≡ (sr-s-eq eqv t) (sr-s-eq eqv s)
sr-s-eq eqv (if t then t₁ else t₂) = if-term-≡ (sr-s-eq eqv t) (sr-s-eq eqv t₁) (sr-s-eq eqv t₂)
...
sr-r-eq eqv (` x) = eqv x
sr-r-eq eqv true = refl
sr-r-eq eqv false = refl
sr-r-eq eqv (t · s) = app-term-≡ (sr-r-eq eqv t) (sr-r-eq eqv s)
sr-r-eq eqv (if t then t₁ else t₂) = if-term-≡ (sr-r-eq eqv t) (sr-r-eq eqv t₁) (sr-r-eq eqv t₂)
...
subst-weaken-idRen' (su x) {s} = lookup (lookupRen x (mapRen su (mapRen su idRen))) (s ∷ (idSub ↑))
                          ...
                          ≡⟨ lookup-idSub (lookupRen x (mapRen su idRen)) ⟩ (` lookupRen x (mapRen su idRen)) ∎
